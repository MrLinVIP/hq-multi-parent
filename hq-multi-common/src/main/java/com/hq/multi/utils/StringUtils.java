...
public final class StringUtils {
     ...
     * @param max 所允许的总长度。max值要大于sub的长度.
     * @param sub 截取后添加字符。
     * @return Left part of string.
     */
    public static String clip(String s, int max, String sub) {
        if (s == null) {
            return "";
        }
        if (s.trim().length() == 0) {
        ...
        // string is short or just the size!
        ...
        } else if (last.getBytes().length <= sub.length() + 1) {
            return s.substring(0, len) + last;
        } else {
            return s.substring(0, len) + sub;
        }
    }
